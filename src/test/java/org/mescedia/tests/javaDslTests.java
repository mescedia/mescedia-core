...
public class javaDslTests extends  CamelTestSupport  {
    ...
    private static final String simpleXsltTestResult = "<root xmlns:D96A=\"http://www.ibm.com/dfdl/edi/un/edifact/D96A\" xmlns:srv=\"http://www.ibm.com/dfdl/edi/un/service/4.1\"><!--=======================--><!--input analyser result file--><!--=======================--><interface type=\"inbound\" user=\"test\">AS2-TEST</interface><message type=\"\" version=\"\" content-type=\"application/edifact\"/><sender type=\"\"/><receiver type=\"\"/></root>";
    private static final String routingSlipTestResult = "<root xmlns:D96A=\"http://www.ibm.com/dfdl/edi/un/edifact/D96A\" xmlns:srv=\"http://www.ibm.com/dfdl/edi/un/service/4.1\"><!--=======================--><!--input analyser result file--><!--=======================--><interface type=\"inbound\" user=\"test\">AS2-TEST</interface><message type=\"DESADV\" version=\"D96A\" content-type=\"application/edifact\"/><sender type=\"14\">MESCEDIA-INITIALIZER</sender><receiver type=\"14\">MESCEDIA-INITIALIZER</receiver></root>";
    ...
    @Test
    public void simpleXsltTransformTest() throws Exception {

        getMockEndpoint("mock:result").expectedBodiesReceived(simpleXsltTestResult);
        template.sendBody("direct:simpleXsltTransform", dfdlXmlEdifact);
        assertMockEndpointsSatisfied();
    }

    @Test
    public void routingSlipTest() throws Exception {

        getMockEndpoint("mock:result").expectedBodiesReceived(routingSlipTestResult);
        template.sendBody("direct:routingSlipTransform", edifactD96A);
        assertMockEndpointsSatisfied();
    }
    ...
    void applicationPropertyTest()  {
        ...
        PropertiesComponent pc = new org.apache.camel.component.properties.PropertiesComponent();
        pc.setLocation("application.properties");

        context.setPropertiesComponent(pc);

        String value = "";
        Optional<String> prop = context.getPropertiesComponent().resolveProperty("fs.root");
        if (prop.isPresent()) {
            value = prop.get();
        }
        assertTrue(value.equals("fs.root"));
    }
